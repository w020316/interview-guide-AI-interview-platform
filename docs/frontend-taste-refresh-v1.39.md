...
| **招聘广场** | ① 分栏重排为「国内优先，海外独立」；② **来源筛选（下拉 + chips 两个控件）都按分栏过滤**；③ 跨栏切换清空来源筛选；④ 修复暗色破版配色 |
| **简历分析** | 取件卡片的 App 唤起逻辑重写（详见 `docs/mobile-app-launch-fix.md`） |

### 3.5 走查补记：一个只有真机渲染才能发现的漏改

第 3.4 节招聘广场的第 ② 条最初**只改了来源 chips**，漏了同页面另一个控件——
上方的「全部来源」**下拉框**仍绑定未过滤的 `meta.sources`。后果是：
国内分栏下从下拉里仍能选到「RemoteOK 全球远程」，选中后必然空列表且无任何提示，
正是本轮要消除的那种死路。

**这类问题的教训**：同一份数据（来源清单）被两个控件消费时，
改一个必须检查另一个；**只改 chips 是「看起来改完了」**。
发现方式 = 用桩数据把页面真实渲染出来、逐控件读它的可选项（见 3.6）。
现已统一为同一个 `visibleSources` computed。

### 3.6 真实渲染走查（桩数据，零线上副作用）

简历分析、招聘广场都需要登录，走查时**不注册线上账号、不写任何生产数据**，改用：

1. 本地 dev server；
2. 浏览器里写入一个**本地构造的桩 JWT**（三段式 + 未来 `exp`，通过 `auth.ts` 的格式校验）；
3. `playwright-cli route` 把 `/api/jobs/meta`、`/api/jobs`、`/api/jobs/favorite*`
   打桩成 fixture JSON（**必须全部打桩**：axios 拦截器在 401/403 时会 `clearAuth()` 并跳登录，
   漏一个就会被踢回登录页，看起来像「改了没生效」）；
4. `playwright-cli resize 390 844` 切手机视口，同时断言
   `document.documentElement.scrollWidth <= innerWidth` 查横向溢出。

**实测结果**：桌面与手机视口下，简历分析取件卡片 8 张全部带常驻「打开网页版 →」入口；
招聘广场国内分栏只列国内源、海外分栏只列海外源（下拉与 chips 一致）；
三页均无横向溢出（390px 下 scrollWidth 381）。
