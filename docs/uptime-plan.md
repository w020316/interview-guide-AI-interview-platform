...
1. **保活 + 健康监控**（**2026-09-22 已更换主力方案**）：
   ...
   - ⚠️ **冷启动实测分布**（由保活工作流 30 次运行记录反推，已剔除热态与撞预算的失败运行）：
     17 次成功样本 **最小 209s / 中位 258s / 最大 406s**，另有 8 次未在唤醒预算内完成（≥480s）；
     另有一次直接实测（轮询到首个 200）为 355s。早期记录的 98s 已随版本迭代失效。
     前端唤醒预算按最坏情况设为 8 分钟（`frontend/src/utils/backendWake.ts`）。
   - ⚠️ **冷启动优化的验证方式**：`node scripts/coldstart-report.mjs` ——
     保活工作流的运行时长即冷启动实测值，公共仓库运行记录免鉴权可读，
     改完 JVM 参数后隔几小时跑一次即可看到趋势（注意剔除失败运行、样本 ≥5 次）。
