...
export const CHANGELOG: ChangelogEntry[] = [
  {
    version: '1.44.0',
    date: '2026-09-26',
    title: '版本 1.44.0 · 取件入口不再指向走不通的路',
    items: [
      { text: '简历分析：微信「文件传输助手」的网页版只在电脑浏览器可用，手机上打开会被它自己拒绝。此前在手机上点「微信」会提示「已为你打开网页版入口」，用户点进去只看到一句「请改用电脑浏览器」，回来还是同一个链接。现在手机上会直接说明该入口仅支持电脑，并把入口换成「复制链接到电脑打开」，同时提示可用「选择本机文件」上传', level: 'user' },
      { text: '简历分析：卡片上新增「网页版仅电脑可用」标记，点之前就能看清这条入口在手机上不适用，不用先撞一次墙', level: 'user' },
      { text: '简历分析：在微信、钉钉等内置浏览器里，兜底入口的文案不再写「未自动跳转？」——那种环境里根本没有尝试跳转，这样写会让人以为自己漏了一步', level: 'user' },
      { text: '模拟面试 / 简历：打开一条已被删除的记录时，提示改为直说「该记录不存在」，不再套一句与事实无关的「请求参数错误」', level: 'user' },
      { text: '招聘广场：按招聘类型筛选时，如果传了系统不认识的类型会明确告知可选值，而不是显示「0 个岗位」让人以为真的没有', level: 'user' },
      { text: '账号安全：注册时会拒绝过于简单的密码（常见弱口令、重复字符、连续数字或字母、包含用户名），此前 12345678 这类密码可以直接注册成功', level: 'user' },
      { text: '模拟面试：作答附图上传失败时的提示更准确——如果问题出在存储配置上，会直接说明「重试无效」，不再让你反复点重试', level: 'user' },
      { text: '模拟面试：从「面试历史」继续面试时，答题页会显示本场面试的目标岗位；此前整页都没有这个信息，答到一半会忘记自己在面什么岗位', level: 'user' },
      { text: '模拟面试：复盘报告抬头会带上本场岗位名，转发给他人时能看出这是面什么岗位的报告（分享卡片与导出 PDF 一直有，只有报告本体漏了）', level: 'user' },
      { text: '管理后台：系统指标里没有样本的项现在统一显示「—」，不再出现「P95 耗时 0 毫秒」这种与旁边「平均耗时 8659 毫秒」自相矛盾的读数；AI 耗时分布改为由服务端自己统计，不再依赖一个在线上读不出值的底层行为', level: 'user' },
      { text: '运维：修复作答附图上传始终失败的问题——部署环境里存储地址末尾多了一个换行符，导致上传请求打向一个不存在的主机名。现已对存储地址与密钥统一做清洗，配置检查与真实请求使用同一个值', level: 'tech' },
      { text: '运维：修复 AI 兜底模型「配了却等于没配」——该模型默认开启思考模式，返回的正文是空的，系统会自动跳过它继续降级，于是每次主模型故障都要白撞一次；现在显式关闭思考模式，兜底才真正生效（实测从「4.25 秒后拿到空内容」变成「1.38 秒拿到正常内容」）', level: 'tech' },
      { text: '运维可观测：存储健康状态此前只看「配置填没填」，即使上传全部失败也显示正常。现在最近一次上传失败会如实反映为「降级」，运维不必再靠猜', level: 'tech' },
    ],
  },
  {
    version: '1.43.0',
